...
- 入力処理は `proconio` クレートに統一しており、`src/main.rs` でも `input!` マクロを使うことを推奨します。
...
### サンドボックス実行時の注意
`cargo build`、`cargo run`、`pahcer run` はいずれも `target/` 以下に大量のファイルを書き込むため、サンドボックス環境だと `Invalid cross-device link` や `Permission denied` で失敗する可能性があります。これらのコマンドを実行する際は、必ずサンドボックス外（エスカレーションあり）の権限で動かしてください。
